...
// 
typedef struct _rect2i { i32 left, top, width, height; } rect2i;
...
#define deb_v2i(src) printf("[%s] x[%d] y[%d]\n", #src, src.x, src.y);
...
// u32
void* base_memset_u32(void* dest, const uint32_t val, size_t len)
{ uint32_t *ptr = (uint32_t *)dest; while (len--) { *ptr++ = val; } return dest; }
...
// real and ptr
t_input_mouse_proto input_mouse; 
t_input_mouse_proto *ptr_imouse = &input_mouse;

// KBD
// https://learn.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
typedef struct t_input_kbd
{
     b32 key_r;     // 0x52
     b32 tgl_key_r;     // 
     b32 key_w;     // 0x57
     ...
     //
     b32 key_escape; // VK_ESCAPE
     
} t_input_kbd_proto;

t_input_kbd_proto input_kbd;
t_input_kbd_proto *ptr_kbd = &input_kbd;
