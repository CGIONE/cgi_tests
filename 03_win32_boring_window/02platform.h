...
BOOL platformWindow()
{
    ...
    // get primary monitor size and start window size, position rect at center
     POINT monitorSize   = (POINT){GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN)};
     POINT startSize     = (POINT){ monitorSize.x - monitorSize.x / 3, monitorSize.y - monitorSize.y / 3};  

     ptf->rectMain = (RECT){
          (monitorSize.x - startSize.x) / 2,
          (monitorSize.y - startSize.y) / 2,
          startSize.x, 
          startSize.y 
     };
...
//
void platformBlit(void *srcBuffer)
{
     GetClientRect(ptf->hwndMain, &ptf->rectMain);

     int rWidth = ptf->rectMain.right - ptf->rectMain.left;
     int rHeight = ptf->rectMain.bottom - ptf->rectMain.top;
