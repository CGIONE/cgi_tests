...
typedef int (WINAPI *GDI_SetStretchBltMode)(HDC hdc, int mode);
GDI_SetStretchBltMode FN_SetStretchBltMode;
...
#pragma GCC diagnostic ignored "-Wcast-function-type"
     ...
     FN_SetStretchBltMode = (GDI_SetStretchBltMode)GetProcAddress(ptf->hLibGdi, "SetStretchBltMode");
     if (FN_SetStretchBltMode == NULL)
     {
          w32terminal_log(TERR, "FN_SetStretchBltMode");
          return FALSE;
     }
...
// The "Zen" fix: Use mode 3 (COLORONCOLOR) to kill the Yellow/White bullshit
     // This ensures that even if there is a 0.0001% sub-pixel shift, 
     // GDI will pick the nearest color rather than averaging them.
     //FN_SetStretchBltMode(ptf->hdcMain, 3); // 3 = COLORONCOLOR
     ...
     deb_i32((i32)bufferX);
     deb_i32((i32)bufferY);
     deb_i32((i32)structBMP.bmiHeader.biWidth);
     deb_i32((i32)structBMP.bmiHeader.biHeight);
