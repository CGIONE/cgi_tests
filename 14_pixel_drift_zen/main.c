...
SWSTART(&stopw_logic);
                    ...
                    // exit on esc key or btn
                    if(ptr_kbd->key_escape == TRUE) 
                    {
                         PostMessage(ptf->hwndMain, WM_CLOSE, 0, 0);
                    } 
...
SWSTART(&stopw_render);
               ...
               //blit
               platformBlit(game->ptr_main, game->buffer_xy.x, game->buffer_xy.y);
