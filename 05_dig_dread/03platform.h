...
//
#define PTF_NAME "05_THE DIGITAL_DREAD_OF_DEBUGGING_THE_FLOW"
...
BOOL platformWindow()
{
    ...
    // get primary monitor size and start window size, position rect at center
     ...
     // simplest way - get number of logical cores
     SYSTEM_INFO sysInfoOne;
     GetNativeSystemInfo(&sysInfoOne);
     ptf->num_log_cores = (INT)sysInfoOne.dwNumberOfProcessors;
     ...
     // total in MB = total in bytes / 1012 / 1024
     ptf->total_mem_mb = (UINT)((FLOAT)statex.ullTotalPhys / 1024.f / 1024.f);
